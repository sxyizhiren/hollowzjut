...
var Foot=function(){
     ...
     * @param state
     */
    var filteTags=function(state){
        if(state && state.content){
            //过滤原状态中的@链接
            state.content=state.content.replace(/<a href=.+?<\/a>/g,'').replace(/<img .+?\/>/g,'').replace(/<span.+?<\/span>/g,'');
        }
    }
    /**
     * 向外推送新状态信息
	...
	*/
	var pushOutText=function(text){
        //不再每一条都加后缀了
		//var description='「棱镜」'+'http://page.renren.com/601715540/channel-noteshow-909327382';
		callbackFn(FootType,msgType.textType,text);
	}
