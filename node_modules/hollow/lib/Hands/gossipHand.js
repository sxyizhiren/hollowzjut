...
var Hand=function(){
    ...
    var hideCode=/[#|＃]匿[#|＃]/g;          //用之前要把lastIndex置为0，不然第二次test或exec会失败
    ...
    var getGossipCallback=function(pageId,gsList){
        //console.log('getGossipCallbackEn,'+pageId);
        ...
        //console.log('getGossipCallbackOut,'+pageId);
    ...
    function commFilter(msg){
        return msg.replace(/树洞姐姐/g,'树洞爷爷')                        //性别矫正
            .replace(/(http:\/\/)?([\w]{1,10}\.){1,3}(com|net|cn|org)/gi,dropTag)  //留言板中的连接,也包含没转成短域名的
