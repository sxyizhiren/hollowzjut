...
var Hand=function(){
    ...
    var talkByReplyListener=function(msg){
        ...
        if((typeof msg == 'object') && ((msg.type == NOTIFY_TYPEID.REPLY16) || (msg.type == NOTIFY_TYPEID.REPLY142))){
            var content=msg.comm.trim();
            var replyTag='回复'+myName;
            if(content.indexOf(replyTag) >= 0){
                console.log('Found Reply to Me.');
                content=content.replace(replyTag,'').replace(/:|：/,'');
                Step(
                    function(){
                ...
                //直接回复状态的和回复他人的直接不回，不然太烦
                console.log('Found Reply to Other.');
            }
        }
    }
    ...
    var notifyListeners={
        ...
        16:[talkByReplyListener],//reply
        142:[talkByReplyListener] //reply also
    };
}
