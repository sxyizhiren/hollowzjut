...
var Hand=function(){
    ...
    var sofaTimesZJUTWallKey='SofaZJUT表白墙';      //存在redis中的key
    var sofaTimesZJUTWall;               // 沙发次数，只对表白墙抢沙发
    ...
    this.init=function(loginInfo,pageList){
        ...
        cookieExpireTimes=0;
        //读取历史沙发次数
        redisClient.get(sofaTimesZJUTWallKey,function(err,num){
            if(num){
                console.log('Last Sofa Time:'+num);
                sofaTimesZJUTWall = parseInt(num);
            }else{
                ...
                if(k.id > stateSourceMap[pageId].lastId){//id有增长，说明有新的state
                    ...
                    if(stateSourceMap[pageId].lastId === 0){
                        ...
                        if(stateSourceMap[pageId].sofa){
                            ...
                            if(typeof sofaTimesZJUTWall === 'number'){
                                replyMsg += '(第' + sofaTimesZJUTWall + '次沙发)';
                                ///update sofa times
                                sofaTimesZJUTWall++;
                                redisClient.set(sofaTimesZJUTWallKey,sofaTimesZJUTWall);
                            }
                            replyMsg  += timeTail;
                            k.replyMsg=replyMsg;
